...
@section('page_title', 'Item Sale Report')
@section('page_subtitle', 'Filter sold items by product, date, customer, branch, terminal, and order details.')
    ...
    <div class="space-y-6">
        <section class="rounded-lg border border-erp-line bg-white shadow-sm">
            <div class="border-b border-erp-line px-5 py-4">
                ...
                <p class="mt-1 text-sm text-erp-mute">Select a date range to run the report.</p>
            </div>
            <form name="searchFormItemSale" class="grid gap-4 p-5 md:grid-cols-12">
                @csrf
                @include('v2.reports.partials.item-sale-filters', ['includeDepartment' => false])
                <div class="flex flex-wrap items-end gap-2 md:col-span-12">
                    ...
                    <a href="{{ url('reports/item-sale-report') }}" class="inline-flex h-10 items-center rounded-lg border border-erp-line px-4 text-sm font-bold text-erp-text transition hover:border-erp hover:text-erp-dark">Reset</a>
        ...
        <section class="rounded-lg border border-erp-line bg-white shadow-sm">
            <div class="border-b border-erp-line px-5 py-4">
                <h2 class="text-base font-bold text-erp-ink">Item Sales</h2>
...
@push('scripts')
    @include('v2.reports.partials.item-sale-scripts', [
        'searchRoute' => route('SrchISReport'),
        'excelUrl' => url('reports/excel-export-item-sale-report'),
