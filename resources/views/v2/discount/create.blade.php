...
@section('content')
<div class="v2-discount-page w-full">
<section class="panels-wells w-full p-t-30 m-t-10 sm-m-t-50">

<div class="v2-discount-grid">
<form id="discountform" class="v2-discount-form-panel" method="post" action="{{url('save-discount')}}">
  @method('post')
  @csrf
  ...
  <div id="daysdiv"></div>
  <div class="v2-discount-form-stack">
...
<!--Applies to-->
    <div class="card" id="divappliesTo">
      ...
      <div class="card-block">
        ...
        <div class="col-lg-12 col-md-12">
          <div class="v2-radio-list">
            <label class="v2-radio-row" for="entire">
              <input type="radio" name="radio" checked="checked" id="entire" value="entire order" />
              <span>Entire Order</span>
            </label>
            <label class="v2-radio-row" for="collection">
              <input type="radio" name="radio" id="collection" value="selected collections" />
              <span>Selected Departments</span>
            </label>
            <label class="v2-radio-row" for="product">
              <input type="radio" name="radio" id="product" value="selected products" />
              <span>Selected Products</span>
              <button type="button" class="v2-inline-action" onclick="$('#product').prop('checked', true).trigger('change');">Show Products</button>
            </label>
          </div>
        </div>
        <!-- RADIO BUTTONS ENDS -->
...
<!--/Applies to-->
    ...
    <div class="card v2-options-card">
      <div class="card-header">
    ...
    <div class="card">
      ...
      <div class="card-block">
        <!-- RADIO BUTTONS STARTS -->
        <div class="col-lg-12 col-md-12">
          <div class="v2-radio-list" id="different">
            <label class="v2-radio-row" for="everyone">
              <input type="radio" name="customerradio" checked="checked" id="everyone" value="1" />
              <span>Everyone</span>
            </label>
            <label class="v2-radio-row" for="specificcustomer">
              <input type="radio" name="customerradio" id="specificcustomer" value="2" />
    ...
    <div class="card v2-days-card">
      <div class="card-header v2-days-header">
        <h5 class="card-header-text">Select Days</h5>

        <div class="rkmd-checkbox checkbox-rotate checkbox-ripple v2-select-all-days">
          <label class="input-checkbox checkbox-primary">
    ...
    <div class="card v2-discount-actions-card">
...
<div class="card v2-discount-summary-panel">
  <div class="card-header">
...
<div class="modal fade modal-flex" id="details-modal" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-md v2-products-dialog" role="document">
...
<script type="text/javascript">
  ...
  function syncRadioRows(groupName) {
    $('input[type="radio"][name="' + groupName + '"]').each(function() {
      $(this).closest('.v2-radio-row').toggleClass('is-selected', this.checked);
    });
  }

  $(document).on('click', '.v2-radio-row', function(e) {
    if ($(e.target).closest('.v2-inline-action').length) {
      return;
    }
    ...
    if (!$input.prop('checked')) {
      ...
      syncRadioRows($input.attr('name'));
    }
  });

  $('input[type="radio"][name="radio"], input[type="radio"][name="customerradio"]').on('change', function() {
    syncRadioRows($(this).attr('name'));
    if ($(this).attr('name') == 'radio') {
      radiochange(this.id);
      return;
    }

    customerchange(this.id);
  });

  syncRadioRows('radio');
  syncRadioRows('customerradio');
  ...
  $(document).on('click', '.v2-discount-page .rkmd-checkbox .captions, .v2-discount-page .rkmd-checkbox .captions2', function() {
    var $checkbox = $(this).siblings('label').find('input[type="checkbox"]');
    if ($checkbox.length) {
  ...
  function loadBuyOneGetOneProducts() {
    $.ajax({
      ...
      success: function(result) {
        ...
        refreshSelect2("#ddlbuyProducts, #ddlgetProducts");
      }
    });
  }
...
$('#details-modal').on('hidden.bs.modal', function () {
  ...
  function selectedCheckbox(id, val) {
    ...
    if ($('#' + id).is(':checked')) {
      if (jQuery.inArray(id, daysArray) === -1) {
        daysArray.push(id);
      }
