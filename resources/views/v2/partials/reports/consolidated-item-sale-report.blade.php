            ...
            <tbody id="itemSaleRows" class="divide-y divide-slate-100">
                @forelse($record as $value)
                    <tr class="hover:bg-slate-50">
                        <td class="px-5 py-4 text-erp-text">{{ !empty($value->inventory) ? $value->inventory->item_code : $value->order->id }}</td>
                        <td class="px-5 py-4 font-semibold text-erp-ink">{{ !empty($value->inventory) ? $value->inventory->product_name : 'NA' }}</td>
                        <td class="px-5 py-4 text-right text-erp-text">{{ $value->total_qty ?? 0.0 }}</td>
                        <td class="px-5 py-4 text-right font-bold text-erp-ink">{{ number_format($value->total_amount ?? 0.0, 2) }}</td>
                ...
                @empty
                    <tr><td colspan="7" class="px-5 py-12 text-center text-erp-mute">No items found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
