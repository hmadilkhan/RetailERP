...
@section('page_title','Update Purchase Order')
@section('page_subtitle','Update draft purchase order details in the V2 workspace with existing item, tax, and totals behavior preserved.')
...
@section('css_code')
    @include('v2.purchase.partials.premium-style')
@endsection
    ...
    <div class="card purchase-card">
         ...
         <div class="card-block">
          <input type="hidden" id="hidd_amt" name="hidd_amt">
           <input type="hidden" id="id" name="id" value="{{$general[0]->purchase_id}}">
           <input type="hidden" id="tax_value" name="tax_value" value="">
           <input type="hidden" id="edit_id" name="edit_id" value="">
           <input type="hidden" id="status_id" name="status_id" value="{{$general[0]->status_id}}">
            <div class="row">
                <!-- vendor select box -->
                            ...
                            <option value="">Select Vendor</option>
                             @if($vendors)
                              @foreach($vendors as $val)
                                @if($val->id == $general[0]->vendor_id)
                                  <option selected value="{{$val->id}}">{{ $val->vendor_name }}</option>
                                @else
                                  <option value="{{$val->id}}">{{ $val->vendor_name." - " }}<b class="f-w-900">{{$val->company_name }}</option>
                                @endif
                              @endforeach
                             @endif
                        </select>
                ...
                <div class="col-lg-4">
                  <div class="form-group">
                    <label class="form-control-label">Date</label>
                        <input type='text' class="form-control" id="date" name="date" value="{{$general[0]->order_date}}" placeholder="DD-MM-YYYY"/>  
                ...
                <div class="col-lg-4">
                  <div class="form-group">
                    <label class="form-control-label">Receipt Date</label>
                            <input type='text' class="form-control" id="rpdate" name="rpdate" value="{{$general[0]->delivery_date}}" placeholder="DD-MM-YYYY"/>  
             ...
             <div class="row">
                ...
                <div class="col-lg-4">
                    <div class="form-group">
                      ...
                      <input type="text" id="ref" name="ref" class="form-control" value="{{$general[0]->refrence}}" />
                    ...
                    <div class="form-group">
                          <label class="form-control-label">Tax</label>
                           <select class="select2 form-control" onchange="taxchange()" data-placeholder="Select Tax" id="tax" name="tax">
                            @if($tax)
                              @foreach($tax as $val)
                                @if($val->id == $general[0]->tax_id)
                                  <option selected value="{{$val->id}}">{{$val->name.' '.$val->value.'%'}}</option>
                                @else
                        ...
                        <select class="form-control select2" data-placeholder="Select Branch" id="branch" name="branch">
                            <option >Select Branch</option>
                             @if($branch)
                                  ...
                                  @if($val->branch_id == $general[0]->branch_id)
            ...
            <div class="row">
                ...
                <div class="form-group">
                  ...
                  <select onchange="taxchange()" class="form-control" id="discount_by" name="discount_by" style="height: 2.2rem !important" >
                    <option value="1" <?php echo $accounts[0]->discount_by == 1 ?'selected':'' ?>>Percentage</option>
                    <option value="2" <?php echo $accounts[0]->discount_by == 2 ?'selected':'' ?>>Amount</option>
                  </select>
                </div>
              </div>
              <!-- <div class="col-lg-2  col-sm-12">
                 ...
                 <div class="col-lg-4  col-sm-12">
                    <div class="form-group">
                        ...
                        <input type="text" onkeyup="otherExpenses()" id="delivery" name="delivery" class="form-control" value="{{$accounts[0]->shipment}}"  />
                        <span class="help-block text-danger" id="taxbox"></span> 
                    </div>       
                </div>
                ...
                <div class="col-lg-4 col-sm-12">
                    <div class="form-group">
                        ...
                        <input type='text' class="form-control" id="payment" name="payment" placeholder="DD-MM-YYYY" value="{{$general[0]->payment_date}}"/>
                        <span class="help-block text-danger" id="rpbox"></span>
                    </div>
                </div>
                ...
                <div class="col-lg-2  col-sm-12">
                    <div class="form-group">
                         <label>Discount (<span id="discbyperitem">By Percentage</span>)</label>
                           <input type="text" onkeypress="return isNumberKey(event)" placeholder="0" name="discount_per_item" id="discount-per-item" class="form-control" onkeyup="discount_change()" />
                        <span class="help-block"></span>
                    </div>       
                </div> 
                 <!-- Tax -->
                ...
                <div class="col-lg-2  col-sm-12">
                    <div class="form-group">
                         <label class="text-danger">Unit Cost</label>
                           <input type="text" name="amount" readonly placeholder="0" id="amount" class="form-control text-danger" />
                        <span class="help-block"></span>
                    </div>       
                </div> 
                  <!-- qty select box -->
...
<div class="row m-t-40">
    <div class="col-lg-8">
        <div class="form-group">
            <textarea class="form-control" rows="3" placeholder="Comments" id="comments" name="comments">{{$general[0]->comments}}</textarea>
        </div>
  ...
  <div class="col-lg-12 col-sm-12 ">
    <div class="form-group ">
      ...
      <button type="button" id="btnDraft"  class="btn btn-md btn-default waves-effect waves-light m-t-25 m-r-25 f-right" >
        <i class="icofont icofont-save"> </i>Save as draft
      </button>
    </div>       
  </div>  
</div>  
...
<script type="text/javascript">
  ...
  function loadVendorProduct()
  {
   ...
   getAccounts('{{$general[0]->purchase_id}}');
            ...
            if($('#product').val() == ""){
                ...
                if(mode == "insert"){
                    $.ajax({
                        ...
                        success : function(result){
                            if(result == 1){
                                swal_alert("Alert!","Item Already Exists. Please Update ","error",false);
                                taxchange();
                                emptyFields();
                            }else{
                                var getid = $('#id').val();
   ...
   function setPerItemTaxCheckCal(){
    var calc = 0;
      ...
      var gross = parseFloat($('#price').val()); 
    ...
    function taxchange(){
      ...
      calc = "";
      var grossamt = $('#gross').html().replace(",", "");
    ...
    function setValues(gross){
        ...
        calc = ((parseInt(gross_total) / 100) * $('#tax_value').val());


        if($('#tax_value').val() != ""){
            gross_total = gross_total + parseInt($('#hidd_amt').val());
        }
        ...
        $.ajax({
                ...
                success : function(result){
                    ...
                    $.each(result, function( index, value ) {
                        ...
                        unitCost = ((parseFloat(value.price) + parseFloat(value.tax_per_item_value)) - discount);
                    ...
                    $('#hSTaxAmount').text(totalTaxAmnt.toFixed(2));
                        ...
                        success:function(resp){
                            if(resp == 1){
                                 swal({
                                       ...
                                       if(isConfirm){
                                         getAccounts('{{$purchaseID}}');
                                         getAccDetails('{{$purchaseID}}');
                                       }
                                   });
                             }
                        }
        ...
        $.ajax({
				...
				console.log(result)
                  ...
                  $('#tax_value').val(matches?matches[0]:0);
                    ...
                    $('#disc').html(parseFloat($('#discount').val()).toLocaleString());
					
					let delvrtAmt = parseFloat($('#delivery').val()).toFixed(2);
                    $('#deliveryamt').html(delvrtAmt.toLocaleString());
					
                    $('#gross').html(parseFloat(result.unitCost).toLocaleString());
                    calc = ((parseFloat(result) / 100) * parseFloat(taxVal));
    ...
    $('#btnFinalSubmit').click(function(){
        ...
        if($('#vendor').val() == ""){
           ...
           swal_alert("Alert!","Date Field Required ","error",false);
        }else if($('#rpdate').val() == ""){
           swal_alert("Alert!","Receiving Date Field Required ","error",false);
        // }else if($('#ref').val() == ""){
        //    swal_alert("Alert!","Reference Field Required ","error",false);
           ...
           $.ajax({
            url : "{{url('/FinalSubmit')}}",
            type : "POST",
            data : {_token : "{{csrf_token()}}",vendor:$('#vendor').val(),branch:$('#branch').val(),tax:$('#tax').val(),date:$('#date').val(),ref:$('#ref').val(),rpdate:$('#rpdate').val(),comments:$('#comments').val(),total_amount:grossamt,taxAmount:taxamt,net_amount:netamt,ID:$('#id').val(),discount:discountAmount,delivery:$('#delivery').val(),payment:$('#payment').val(),status:$('#status_id').val()},
            success : function(result){
                console.log(result);
                // return false;
                window.location = "{{ url('/view-purchases') }}";
    ...
    $('#btnDraft').click(function(){
        var grossamt = $('#gross').html().replace(",", "");
        var taxamt = '';//$('#taxAmount').html().replace(",", "");
        var netamt = $('#net_amount').html().replace(",", "");
       $.ajax({
            url : "{{url('/Draft')}}",
            type : "POST",
            data : {_token : "{{csrf_token()}}",vendor:$('#vendor').val(),branch:$('#branch').val(),tax:$('#tax').val(),date:$('#date').val(),ref:$('#ref').val(),rpdate:$('#rpdate').val(),comments:$('#comments').val(),total_amount:grossamt,taxAmount:taxamt,net_amount:netamt,ID:'{{$purchaseID}}',discount:$('#discount').val(),discount_by:$('#discount_by').val(),delivery:$('#delivery').val(),payment:$('#payment').val()},
            success : function(result){
                window.location = "{{ url('/view-purchases') }}";
            }
       });
