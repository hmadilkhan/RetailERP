...
@section('breadcrumtitle','Edit Vehicle')
...
@section('content')
	<section class="panels-wells">
		...
		<div class="card-header">
			<h5 class="card-header-text"> Edit Vehicle</h5>
		...
		<div class="card-block">
			<form method="POST" action="{{ route('vehicle.update') }}" class="form-horizontal" enctype="multipart/form-data">
			@csrf
				...
				<div class="row">
					<div class="col-md-4">
						<div class="form-group {{ $errors->has('model_name') ? 'has-danger' : '' }} ">
							...
							<input type="text" name="model_name" id="model_name" placeholder="Vehicle Model Name" class="form-control" value="{{ $vehicle->model_name }}"/>
					...
					<div class="col-md-4">
						<div class="form-group {{ $errors->has('model_no') ? 'has-danger' : '' }} ">
							...
							<input type="text" name="model_no" id="model_no" placeholder="Vehicle Model No." class="form-control" value="{{ $vehicle->model_no }}"/>
					...
					<div class="col-md-4">
						<div class="form-group {{ $errors->has('mobile') ? 'has-danger' : '' }} ">
							...
							<input type="text" name="number" id="number" placeholder="Vehicle Number" class="form-control" value="{{ $vehicle->number }}"/>
					...
					<div class="contact-card-button m-t-20 f-right">
						<button type="submit" class="btn btn-md btn-info waves-effect waves-light">
						  <i class="icofont icofont-plus m-r-5"> </i>Update Vehicle
						</button>
					</div>  
				</div>
			</form>
