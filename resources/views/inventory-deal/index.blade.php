                            ...
                            <div class="form-group {{ $errors->has('deal') ? 'has-danger' : '' }}">
                                ...
                                <select name="deal" id="deal"  class="form-control select2">
                                    ...
                                    @foreach($DealProducts as $val)
                                      <option {{ old('deal') == $val->id ? 'selected' : '' }} value="{{ $val->id }}">{{ $val->product_name }}</option>
                                    @endforeach
                                </select>
                                 @if ($errors->has('deal'))
                       ...
                       <br/>
                         <div class="form-inline">
                            <div class="form-group {{ $errors->has('group') ? 'has-danger' : '' }}">
                                ...
                                <select name="group" id="group"  class="form-control select2" >
                                    ...
                                    @foreach($InventGroups as $val)
                                      <option {{ old('group') == $val->id ? 'selected' : '' }} value="{{ $val->id }}">{{ $val->name }}</option>
                                    @endforeach
                                </select>
                                 @if ($errors->has('group'))
                                    <div class="form-control-feedback">Required field can not be blank.</div>
                            ...
                            <div class="form-group m-l-5">
                                ...
                                <select name="products[]" id="products" multiple="multiple" class="form-control select2" style="width: 75%"></select>
                          ...
                          @foreach($getRecord2 as $list_val)
                             @if(count($list_val->deals) > 0)
                              <tr>
                                  ...
                                  <td class="pointer" onclick="getDetail({{ $list_val->id }},'{{ $list_val->deals }}','{{ addslashes($list_val->product_name) }}')" >
                                      @foreach($list_val->deals as $list_val2)
                                       <label class="badge badge-default pointer"> {{ $list_val2->inventoryGroup->name }} </label>
                                      @endforeach
                                  </td>
                                  <td>
                                     <i onclick="deleteDeal('{{$list_val->id}}','{{ $list_val->product_name }}')" class="text-danger text-center icofont icofont-trash" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete Deal"></i> 
                                     
                                     <form id="removeDeal{{ $list_val->id }}" action="{{ route('dealRemove') }}" method="post">
                                         @csrf
       ...
       <div class="modal-dialog modal-md" role="document">
          <div class="modal-content">
             ...
             <div class="modal-footer">
                <button type="submit" class="btn btn-primary waves-effect waves-light" id="update_md">Update</button>
             </div>
            </form> 
          </div>
           </div>
                     ...
                     if(resp != ''){
                        if(md == 0){  
                         ...
                         $.each(resp,function(i,v){
                             $("#products").append('<option selected value="'+v.id+'">'+v.name+'</option>');
                         })
                        }else{
                           tmp = resp;
                        }
...
;
		...
		$.each(deals,function(index,value){
		    var click = 'onclick="deleteDealDetail('+value.inventory_group.id+','+id+')"';
               ...
               $("#md_body").append('<h3 id="dealCategory'+value.inventory_group.id+'" class="displayInline">'+value.inventory_group.name+'  </h3> <a href="javascript:void(0)" title="Remove '+value.inventory_group.name+'" '+click+' class="text-danger f-right"> <i class="icofont icofont-trash"></i> </a> ');
               $("#md_body").append('<div class="form-group"><label>Select Products</label><select id="md_product_'+value.inventory_group.id+'" name="'+value.inventory_group.id+'[]" multiple="multiple" class="form-control"></select></div>');
               getProducts(value.inventory_group.id,1);
               $("#md_product_"+value.inventory_group.id).select2(); 
               
              $.each(value.get_deal_details,function(i){
                 $("#md_product_"+value.inventory_group.id).append('<option selected value="'+tmp[i].id+'">'+tmp[i].name+'</option>');
              });
		})        
    }
	...
	{		
		swal({
                ...
                if(isConfirm){
                    ...
                    swal({
                        title: "Cancel!",
                        text: $(".modal-title").text()+" ("+$("#dealCategory"+id).text()+") are safe :)",
                        type: "error"
                    },function(isConfirm){
                        if(isConfirm){
                            window.location="{{route('listInventDeal')}}";
