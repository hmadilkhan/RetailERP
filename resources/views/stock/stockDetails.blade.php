...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            ...
            <a href="{{ url('/stock-list') }}" id="btnback" name="btnback"><i class="icofont icofont-arrow-left"></i>Back to
                List
            </a>
                            ...
                            @if ($details)
                                ...
                                @foreach ($details as $detail)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $detail->purchase == 'Sales' ? $detail->qty / $conversion_unit[0]->weight_qty : $detail->qty }}
                                        </td>
                                        <td>{{ $detail->branch_name }}</td>
                                        <td>{{ $detail->purchase == 'Sales' ? $detail->terminal : '' }}</td>

                                        <td>{{ $detail->purchase > 0 ? 'By Purchase' : ($detail->transfer > 0 ? 'By Transfer' : ($detail->purchase == 'Sales' ? 'Sales' : 'Stock Opening')) }}
                                        </td>
                                        <td>
                                            @if ($detail->purchase > 0)
                                                <a href="{{ route('view', $detail->purchase) }}"
                                                    class="p-r-10 f-18 text-info" data-toggle="tooltip"
                                                    data-placement="top" title="" data-original-title="Edit"><i
                                                        class="icofont icofont-eye-alt"></i></a>
                                            @endif
                                            @if ($detail->transfer > 0)
                                                <a href="{{ url('challandetails', $detail->transfer) }}"
                                                    class="p-r-10 f-18 text-warning" data-toggle="tooltip"
                                                    data-placement="top" title="" data-original-title="Edit"><i
                                                        class="icofont icofont-eye-alt"></i></a>
                                            @endif
                                            @if ($detail->purchase == 'Sales')
                                                <a onclick="getBill('{{ $detail->receipt_id }}')"
                                                    class="p-r-10 f-18 text-warning" data-toggle="tooltip"
                                                    data-placement="top" title="" data-original-title="Edit"><i
                                                        class="icofont icofont-eye-alt"></i></a>
                                            @endif
                    ...
                    <div class="row m-t-20">
                        ...
                        <div class="col-md-4">
                            <button type="button" id="fetch"
                                class="btn btn-success waves-effect waves-light m-t-25 f-left">
                                <i class="icofont icofont-ui-check"> </i>Fetch
                            </button>
                            <button type="button" id="pdf"
                                class="btn btn-danger waves-effect waves-light m-t-25 m-l-10 f-left">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if (preg_match('/Purchase/', $value->narration) && $value->adjustment_mode == '')
                                                <a href="{{ route('view', $value->foreign_id) }}"
                                                    class="p-r-10 f-18 text-info" data-toggle="tooltip"
                                            ...
                                            @elseif(preg_match('/Sales Return/', $value->narration))
                                                <a href="{{ url('sales-return', $value->foreign_id) }}"
                                                    class="p-r-10 f-18 text-info" data-toggle="tooltip"
                                                    data-placement="top" title="" data-original-title="Edit"> <i
                                                        class="icofont icofont icofont-printer text-success"></i>
                                                </a>
    ...
    <script type="text/javascript">
        ...
        $('#pdf').click(function(e) {
            ...
            if ($('#fromdate').val() == "") {
                ...
                window.location = "{{ url('stockReportPDF') }}?from=" + $('#fromdate').val() + "&to=" + $(
                    '#todate').val() + "&id={{ $product_id }}"
            }
        });
                        ...
                        success: function(result) {
                            ...
                            for (var count = 0; count < result.length; count++) {
                                $("#tablemodal tbody").append(
                                    ...
                                    "<td '>" + parseInt(result[count].total_amount)
                                    .toLocaleString() + "</td>" +
                                    "</tr>"
                                )
                            }
