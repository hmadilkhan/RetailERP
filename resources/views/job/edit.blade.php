...
@section('content')
    ...
    <form id="customerform" method="POST" class="form-horizontal" enctype="multipart/form-data"  action="{{ route('customer.store') }}">
       @csrf
    	...
    	<input type="hidden" id="hidd_id" name="hidd_id" value="{{$general[0]->recipy_id}}">
              <div class="row">
                        ...
                        <select class="select2" data-placeholder="Select Finished Good" id="finished" name="finished">
                            <option value="">Select Finished Good</option>
                           	@if($products)
                                  ...
                                  @if($value->id == $general[0]->item_id)
                           			    <option  selected="selected" value="{{$value->id}}">{{$value->product_name}}</option>
                 	...
                 	<div class="row">
                          ...
                          <div class="form-group {{ $errors->has('name') ? 'has-danger' : '' }} ">
                           <label class="form-control-label">Processing Cost</label>
                            <input readonly="readonly" type="text" id="cost" name="cost" class="form-control" value="{{ $accounts[0]->cost }}" />
                             @if ($errors->has('name'))
                    ...
                    <div class="col-md-3">
                          <div class="form-group {{ $errors->has('name') ? 'has-danger' : '' }} ">
                           <label class="form-control-label">Infra-Structure Cost</label>
                            <input type="text" id="infra" name="infra" class="form-control" value="{{ $accounts[0]->infrastructure_cost }}" onchange="getInfraCost()" />
                             @if ($errors->has('name'))
                                    <div class="form-control-feedback">Required field can not be blank.</div>
                                @endif
                        </div>
                    ...
                    <div class="col-md-3">
                          <div class="form-group {{ $errors->has('name') ? 'has-danger' : '' }} ">
                           <label class="form-control-label">Total Cost</label>
                            <input readonly="readonly" type="text" id="totalCost" name="totalCost" class="form-control" value="{{ $accounts[0]->cost + $accounts[0]->infrastructure_cost  }}" />
                             @if ($errors->has('name'))
                                    <div class="form-control-feedback">Required field can not be blank.</div>
                                @endif
                        </div>
    ...
    $('#btnSubmit').click(function(e){
    	if($('#finished').val() == ""){
		    	...
		    	$.ajax({
			        url:'{{ url("/add-job") }}',
			        type:"POST",
			        data:{_token : "{{csrf_token()}}",jobid:$('#hidd_id').val(),id:$('#finished').val(),qty:$('#qty').val()},
			        success:function(result){
			        	if (result) 
			        	{
			        		$('#hidd_id').val(result);
...
getDetails();
                    ...
                    $.each(result, function( index, value ) {
                        $("#item_table tbody").append(
                          "<tr>" +
                            ...
                            "<td>"+value.amount+"</td>" +
                            "<td class='action-icon'><i id='btn"+index+"' onclick='updateItem("+value.recipy_details_id+","+value.item_id+","+value.uom_id+","+value.usage_qty+","+value.amount+")'  class='icofont icofont-ui-edit' data-toggle='tooltip' data-placement='top' title='' data-original-title='Edit'></i>"+" &nbsp;"+"<i id='btn"+index+"' onclick='deleteItem("+value.recipy_details_id+")'  class='icofont icofont-ui-delete' data-toggle='tooltip' data-placement='top' title='' data-original-title='Delete'></i></td>" +
                          "</tr>"
                         );
                    });
                }
    ...
    $('#btnFinalSubmit').click(function(e){
    	...
    	$.ajax({
                ...
                type : "POST",
                data : {_token : "{{csrf_token()}}", recipyid:$('#hidd_id').val(),cost:$('#cost').val(),infra:$('#infra').val(),rp:$('#rp').val()},
                dataType : 'json',
                success : function(result){
                   	if (result == 1) 
                   	{
