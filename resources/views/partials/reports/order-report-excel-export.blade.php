		...
		@foreach ($record as $value)
		   ...
		   <tr>
		      ...
		      @else
				...
				<td style="text-align: center;">{{($value->payment_id == 3 ? $value->orderAccount->receive_amount : $value->total_amount)}}</td>  
				<td style="text-align: center;">{{($value->payment_id == 3 ? ($value->actual_amount - $value->orderAccount->receive_amount) : 0)}}</td>  
			  @endif
			  @if(session("company_id") == 102 && $value->orderStatus->order_status_name == "Order Picked Up")
				    @foreach($value->statusLogs as $log)
						...
						<td style="text-align: center;">{{date("d M Y",strtotime($log->date)).' '.date("h:i a",strtotime($log->time))}}</td>
						@endif
					@endforeach
			  @endif
		   </tr>
