@if (isset($record))
    <div class="card card-block">
        <div class="project-table">
            <table id="widget-product-list" class="table table-striped nowrap dt-responsive nowrap" width="100%"
                cellspacing="0">
                <thead>
                    <tr>
                        <th>Code</th>
                ...
                <tbody>
                    @if ($record)
                        @foreach ($record as $value)
                            <tr>
                                <td>{{ !empty($value->inventory) ? $value->inventory->item_code : $value->order->id }}
                                </td>
                                <td class="pro-name">
                                    {{ !empty($value->inventory) ? $value->inventory->product_name : 'NA' }}</td>
                                <td>{{ $value->total_qty ?? 0.0 }}</td>
                                <td>{{ number_format($value->total_amount ?? 0.0, 2) }}</td>
