...
<div class="row dashboard-header m-t-15">
   <div class="col-lg-6 col-md-6" onclick="" style="cursor:pointer;">
      <div class="card dashboard-product">
         ...
         <h2 class="dashboard-total-products f-28">{{(empty($projected) ? 0 : number_format($projected[0]->sales,2))}}</h2>
   ...
   <div class="col-lg-6 col-md-6" onclick="getdetails()" style="cursor:pointer;">
      <div class="card dashboard-product">
         ...
         <h2 class="dashboard-total-products f-28">{{(empty($totalSales) ? 0 : number_format($totalSales[0]->TotalSales,2))}}</h2>
...
<script type="text/javascript">
    var data1 = [
            @foreach($sales as $saleValue)
               { y: '{{$saleValue->terminal_name}}', a: {{$saleValue->cash}},  b: {{$saleValue->creditCard}} , c:{{$saleValue->CustomerCredit }}},
           @endforeach
         ];
         var data2 = [
                @foreach($monthsales as $sales)
             {"x" : '{{$sales->branch_name." (".date('F',strtotime($sales->date)).")"}}',"y" : '{{$sales->total}}'},
             @endforeach
         ];
</script>
<div class="row">
