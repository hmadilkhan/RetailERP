                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-2 col-md-2">
                            <div class="form-group {{ $errors->has('debit') ? 'has-danger' : '' }}">
                                ...
                                <input class="form-control" type="number" name="debit" id="debit" required
                                    value="0" value="{{ old('debit') }}" />
                                @error('debit')
                                    <div class="form-control-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-lg-4 col-md-4">
                            <div class="form-group {{ $errors->has('credit') ? 'has-danger' : '' }}">
                                ...
                                <input class="form-control" type="text" name="narration" id="narration"
                                    value="{{ old('narration') }}" />
                                @error('narration')
                                    <div class="form-control-feedback">{{ $message }}</div>
                                @enderror
                ...
                <div class="row">
                    <div class="col-lg-4 col-md-4">
                        <div class="form-group {{ $errors->has('date') ? 'has-danger' : '' }}">
                            ...
                            <input class="form-control" type="text" name="from" id="fromdate"
                    ...
                    <div class="col-lg-4 col-md-4">
                        <div class="form-group {{ $errors->has('date') ? 'has-danger' : '' }}">
                            ...
                            <input class="form-control" type="text" name="to" id="todate"
                                placeholder="YYYY-MM-DD" />
                            @error('to')
                                <div class="form-control-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-lg-4 col-md-4">
                        ...
                        <div class="button-group ">
                            <button type="button" id="btndraft" onclick="generate_pdf()"
                                class="btn btn-md btn-danger waves-effect waves-light f-left "> <i
    ...
    <section class="panels-wells">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{ url('create-payment', $slug) }}"
                    class="btn btn-primary waves-effect waves-light f-right d-inline-block"> <i
                        class="icofont icofont-plus m-r-5"></i>Make a Payment </a>
                ...
                <div class="project-table">
                            ...
                            @if ($details)
                                @foreach ($details as $value)
                                    <tr>
                                        ...
                                        <td> <a href="{{ url('view', $value->purchase_id) }}">{{ $value->po_no }}</a>
                                        </td>
                                        <td>{{ $value->total_amount }}</td>
                                        <td>{{ number_format($value->debit, 2) }}</td>
                                        <td>{{ number_format($value->credit, 2) }}</td>
                                        <td class="{{ $value->balance >= 0 ? 'text-success' : 'text-danger' }}">
                                            {{ number_format($value->balance, 2) }}</td>
                                        <td
                                        ...
                                        <td>
                                            @if ($value->debit != '' && $value->debit != 0 && $value->purchase_id != '')
                                                <a href="{{ url('vendor-voucher', $value->vendor_account_id) }}"
                                                    ...
                                                    data-original-title="Print Voucher"><i
                                                        class="icofont icofont icofont-print"></i></a>
                                            @else
                                            @endif
                                        </td>
                                    </tr>
                    ...
                    <div class="button-group ">
                        <button type="button" id="btnsubmit"
                            ...
                            onclick="alert('Work in process')"><i class="icofont icofont-file-excel"> </i>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-success waves-effect waves-light"
                        onClick="updatenarration()">Update Narration</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function loadAmount() {
            $.ajax({
                ...
                data: {
                    _token: "{{ csrf_token() }}",
                    id: $("#po").children("option:selected").val()
                },
        ...
        function generate_pdf() {
            if ($("#fromdate").val() == "") {
                ...
                window.location = "{{ url('vendor-ledger-report', $slug) }}" + "/" + $("#fromdate").val() + "/" + $(
                    "#todate").val();
            }
        }
    </script>
