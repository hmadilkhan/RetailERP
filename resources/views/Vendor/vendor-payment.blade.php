				...
				<div class="modal-dialog modal-md" role="document">
					  <div class="modal-content">
						 ...
						 <div class="modal-footer">
							<button type="button" class="btn btn-success waves-effect waves-light" onClick="updateDueDate()">Change Date</button>
						 </div>
					  </div>
			   </div>
			</div> 
			...
			<div class="modal fade modal-flex" id="vendor-payment-history-modal" tabindex="-1" role="dialog">
				<div class="modal-dialog modal-md" role="document">
					  <div class="modal-content">
						 ...
						 <div class="modal-footer">
							<button type="button" class="btn btn-success waves-effect waves-light" onClick="hideModal()">Close</button>
						 </div>
					  </div>
			   </div>
			</div> 
...
@section('scriptcode_three')
	...
	function getData(page)
	{
		$.ajax({
			...
			type: 'GET',
			data:{_token:"{{ csrf_token() }}",from:from,to:$('#to_date').val(),vendor:$('#vendor').val(),mode:modes},
			success:function(resp){
				$("#tablesData").empty();
				$("#tablesData").append(resp);
				// $("#paymentTable tbody").empty();
				// $("#paymentTable tbody").append(resp);
			}
		});
	}
	...
	function updateDueDate(){
		$.ajax({
			...
			type: 'POST',
			dataType:"json",
			data:{_token:"{{ csrf_token() }}",id:$('#purchaseId').val(),date:$('#duedate').val()},
			success:function(resp){
				if(resp.status == 200){
