    ...
    <div class="card-block ">
     <form id="deliveryAreasForm" action="{{ route('deliveryAreaStore') }}" method="post">
      ...
      <div class="row">
        ...
        <div class="col-md-2">
            <div class="form-group">
              <label for="delivery_free_on_min_order" class="form-control-label">Delivery free on minium order</label>
              <input type="text" name="delivery_free_on_min_order" id="delivery_free_on_min_order" class="form-control" value="{{ old('delivery_free_on_min_order') ? old('delivery_free_on_min_order') : 0 }}">
            </div>
          </div>
      </div>
        ...
        <div class="col-md-4">
          <div class="form-group">
            <label for="city" class="form-control-label">City</label>
              ...
              <label class="switch f-right" title="" data-toggle="tooltip" data-placement="top" data-original-title="You want city area click to slide on">
       ...
       @foreach($deliveryList as $parent_rowVal)
          ...
          <!--<td id="minOrder{{-- $value->branch_id --}}" class="pointer" onclick="modifyField({{-- $value->branch_id --}},'{{-- $value->min_order --}}','min_order')">{{-- 'PKR '.$value->min_order --}}</td>-->
				  <td class="action-icon">
                    ...
                    <i class="icofont icofont-edit text-warning f-18 alert-confirm pointer m-r-1" data-toggle="tooltip" data-placement="top" data-original-title="Edit delivery detail" onclick="editMainDetail({{ $parent_rowVal->website_id }},{{ $parent_rowVal->branch_id }},'{{ addslashes($parent_rowVal->branch_name) }}','{{ $parent_rowVal->min_order }}','{{ $parent_rowVal->delivery_free_on_min_order }}','{{ $parent_rowVal->estimate_time }}','{{ $parent_rowVal->estimate_of_days }}')"></i>
...
<script type="text/javascript">
 ...
 $("#btn_modify_deliveryDetail").on('click',function(){
