        ...
        <div class="row bg-transparent m-2 p-2" style="padding-top:-3.9rem;">
                        ...
                        <div class="col-xl-4 ">
                            <div class="  d-flex align-items-start  pb-2">
                                <div class="  bg-transparent py-2 px-4">
                                    <h5 class="font-size-24 text-sm-end text-muted mb-5">Date Time : {{ $orders->dateTime }}
                                    </h5>
                                    <label
                                        class="label {{ Custom_Helper::getColorName($orders->order_status) }} font-size-28 border border-white rounded my-5 mt-3">
                                ...
                                <div class="text-sm-end mt-2 mt-sm-0 float-end">
                                    @if (session('roleId') == 19 or session('roleId') == 2)
                                        @if ($orders->order_status == 'Pending')
                                            <a href="{{ url('/sales/change-order-status-from-website') }}/{{ $orders->id }}/2/{{ $orders->url_orderid }}"
                                        ...
                                        @elseif($orders->order_status == 'Processing')
                                            <a href="{{ url('/sales/change-order-status-from-website') }}/{{ $orders->id }}/3/{{ $orders->url_orderid }}"
                                                class="btn btn-primary  ml-2">
                                                <i class="mdi mdi-cart-outline me-1"></i> Ready </a>
                                        @elseif($orders->order_status == 'Ready for Delivery')
                                            <a href="{{ url('/sales/change-order-status-from-website') }}/{{ $orders->id }}/6/{{ $orders->url_orderid }}"
                            ...
                            @else --}}
                                ...
                                <div class="flex-grow-1 align-self-center overflow-hidden">
                                    <div>
                                        <!-- text-truncate  -->
                                        <h3><a href="#"
                                                class="code{{ $key }} text-dark fw-bold">({{ $item->item_code }})
                                            </a> <a href="#" class="name{{ $key }} text-dark font-size-24">
                                                {{ $item->product_name }} <span
                                                    class="text-muted">{{ $item->department_name ?? '' }}</span>
                                                @if ($item->prod_variation != null && $orders->website_type == 'shopify')
                            ...
                            @if ($item->prod_variation != null && $orders->website_type != 'shopify')
                                <div class="">
                                    @foreach ($item->prod_variation as $variation)
                                        <div>
                                            <h5 class="text-muted font-size-18 m-t-1 m-b-0">{{ $variation->name ?? '' }}
                                            </h5>
                                            @if (count($variation->values) > 0)
                                                @foreach ($variation->values as $variation_val)
                                                    ...
                                                    @foreach ($variation_val->variation as $sbvariat_val)
                                                        <h5 class="text-muted font-size-18 m-t-1 m-b-0">
                                                            {{ $sbvariat_val->name }}
                                                        </h5>
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="mt-3">
                                            ...
                                            @if ($item->discount_value != 0)
                                                <h5 class="mb-0 mt-2">
                                                    <del>{{ session('currency') . ' ' . number_format($item->actual_price, 0) }}</del>
                                                </h5>
                                            @endif
                                            <h5 class="mb-0 mt-2">
                                    ...
                                    <div class="col-md-3">
                                        <div class="mt-3">
                                            ...
                                            <h5>{{ session('currency') . ' ' . number_format($item->webcart_amount, 0) }}
                                            </h5>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="col-xl-4">
                    ...
                    <div class="card border shadow-none">
                        <div class="card-header bg-transparent border-bottom py-3 px-4">
                            <h5 class="font-size-16 mb-0">Order Details <span
                                    class="float-end">#{{ $orders->url_orderid }}</span></h5>
                        </div>
                                    ...
                                    <tbody>
                                        ...
                                        @if ($orders->discount_amount != 0)
                                            <tr>
                                                <td>Discount : </td>
                                                <td class="text-end">-
                                                    {{ session('currency') . ' ' . $orders->discount_amount . ' (' . $orders->discount_percentage . '%)' }}
                                                </td>
                                            </tr>
                                        @endif
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-end">
                                                {{ session('currency') . ' ' . $orders->delivery_charges }}
                                            </td>
                                        </tr>

                                        <tr class="bg-light">
                                            <td>Total Amount :</td>
                                            <td class="text-end">{{ session('currency') . ' ' . $orders->total_amount }}
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                ...
                <div class="mt-5 mt-lg-0">
                    <div class="card border shadow-none">
                        <div class="card-header bg-transparent border-bottom py-3 px-4">
                            <h5 class="font-size-16 mb-0"> Booking Schedule</h5>
                        </div>
                        <div class="card-body p-4 pt-2">
                            @if(empty($orders->pickupDate) && empty($orders->pickupSlot) && empty($orders->deliveryDate) && empty($orders->deliverySlot))
                                <p class="text-muted text-center">No Schedule Found</p>
                            @else
                                <table class="table mb-0">
                                    <tbody>
                                        <tr>
                                            <td>Pickup Date :</td>
                                            <td class="text-end">
                                                {{ $orders->pickupDate != '' ? date('d F Y', strtotime($orders->pickupDate)) : '-' }}
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Pickup Slot :</td>
                                            <td class="text-end">{{ $orders->pickupSlot != '' ? $orders->pickupSlot : '-' }}
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Delivery Date :</td>
                                            <td class="text-end">
                                                {{ $orders->deliveryDate != '' ? date('d F Y', strtotime($orders->deliveryDate)) : '-' }}
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Delivery Slot :</td>
                                            <td class="text-end">
                                                {{ $orders->deliverySlot != '' ? $orders->deliverySlot : '-' }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            @endif
            ...
            <div class="modal-dialog" data-dismiss="modal">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <p class="text-left mt-2  fs-5"><span class="fw-bold">Comments : </span><span id="modalComments"
                                class=" fs-5"></span></p>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" id="btn_extra_item"
