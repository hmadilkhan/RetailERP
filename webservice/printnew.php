...
class PrintReceipt
{
    ...
    public function ReceiptSample($order,$items,$imageUrl)
    {
        ...
        $this->printer->setupColumns(
            [300, SunmiCloudPrinter::ALIGN_LEFT],
        ...
        $this->printer->printInColumns("Product","Price","Qty", "Amount");
        $this->printer->setPrintModes(false, false, false);
        $this->printer->appendText("------------------------------------------");
        $this->printer->lineFeed();
		$totalamount = 0;
        ...
        foreach($items as $item){
			$name = str_split($item["product_name"], 18);
			$totalItems++;
	...
	public function RizwanReceiptSample($order,$items)
    {
		$this->printer->selectAsciiCharFont(1);
        $this->printer->setLineSpacing(12);
        $this->printer->setPrintModes(true, true, false);
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
        $this->printer->appendImage("https://i.ibb.co/YX8qvDf/Rizwan-Traders.jpg", SunmiCloudPrinter::DIFFUSE_DITHER);
		$this->printer->appendText("----------------------------------------------");
        $this->printer->lineFeed();
