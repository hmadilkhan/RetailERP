...
$items = $GLOBALS['crud']->runQuery("SELECT a.receipt_id,a.item_code,c.product_name,a.item_name,a.total_qty,a.item_price,a.total_amount FROM sales_receipt_details a INNER JOIN inventory_general c on c.id = a.item_code  where a.receipt_id = ".$receiptId);
...
$printer->setupColumns(
	[240, SunmiCloudPrinter::ALIGN_LEFT],
	[0, SunmiCloudPrinter::ALIGN_RIGHT],
);
$printer->printInColumns("Payment", $order['payment_mode']." Payment");
$printer->printInColumns("Receipt No", $order['receipt_no']);
$printer->printInColumns("Customer", $order['bill_print_name']);
$printer->printInColumns("Order", $order['id']);
$printer->printInColumns("Attendant", $order['username']." (".$order['userid'].")");
$printer->printInColumns("TID", $order['terminal_name']);
