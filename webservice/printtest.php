...
class SunmiCloudPrinter
{
    ...
    private const APP_ID = "016bf822947849a8a13a46f037d16329";
    private const APP_KEY = "25f65000aba44532b7310f337a5a435b";
    ...
    function pushVoice($sn, $content, $url, $cycle = 1, $interval = 2, $expire_in = 300)
    {
         $body = [
            "sn"        => $sn,
            "content"   => $content,
    ...
    function setPrintModesCustom($bold, $double_h, $double_w)
    {
        $n = 0;
        if ($bold)
            $n |= 32;
...
class PrintReceipt
{
    ...
    public function __construct($dots_per_line = 500,$serial)
    {
        $this->printer = new SunmiCloudPrinter($dots_per_line);
        $this->serial_number = $serial;
    }
	
	public function ReceiptFiver($order,$items,$imageUrl)
    {
		// $this->printer->selectAsciiCharFont(50);
        $this->printer->setLineSpacing(12);
        $this->printer->setPrintModes(true, true, false);
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_RIGHT);
        // $this->printer->appendImage($imageUrl, SunmiCloudPrinter::DIFFUSE_DITHER);
        $this->printer->lineFeed();
		$this->printer->setAlignment(SunmiCloudPrinter::ALIGN_LEFT);
		$this->printer->setupColumns(
            [360, SunmiCloudPrinter::ALIGN_LEFT,1]
        );

		// $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
		// $this->printer->setPrintModes(true, true, true);
		// $this->printer->printInColumns("DELIVERY",0);
		// $this->printer->lineFeed();
        $this->printer->setupColumns(
        ...
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_LEFT);
		$this->printer->setPrintModes(true, true, true);
		$this->printer->setUnderlineMode(10);
		$this->printer->setBlackWhiteReverseMode(0);
		$this->printer->printInColumns("Andy Vockins");
		$this->printer->lineFeed();
		$this->printer->printInColumns("110 Bramley Crescent");
		$this->printer->lineFeed();
		$this->printer->printInColumns("0742 517580");
		$this->printer->lineFeed();
		...
		// $this->printer->setPrintModes(true, true, false);
		// $this->printer->printInColumns("1 x LARGE LAMB DONER KABAB", "8.50");
		// $this->printer->lineFeed();
		// $this->printer->setPrintModes(false, true, false);
		// $this->printer->printInColumns("All Salad", "");
		// $this->printer->lineFeed();
		// $this->printer->printInColumns("Chilli Sauce", "");
		// $this->printer->lineFeed();
		...
		// $this->printer->setPrintModes(true, true, false);
		// $this->printer->printInColumns("1 x SMALL LAMB DONER KABAB", "8.50");
		// $this->printer->lineFeed();
		// $this->printer->setPrintModes(false, true, false);
		// $this->printer->printInColumns("No Salad", "");
		// $this->printer->lineFeed();
		// $this->printer->printInColumns("No Sauce", "");
		// $this->printer->lineFeed();
		// $this->printer->setPrintModes(true, false, false);
		// $this->printer->appendText("----------------------------------------------");
		// $this->printer->lineFeed();
        ...
        // $this->printer->setPrintModes(true, true, false);
		// $this->printer->printInColumns("1 x LARGE CHIPS", "3.30");
		// $this->printer->lineFeed();
        // $this->printer->setPrintModes(true, false, false);
        // $this->printer->appendText("----------------------------------------------");
		// $this->printer->lineFeed();
        ...
        // $this->printer->setPrintModes(false, false, false);
		// $this->printer->printInColumns("SUB TOTAL", "17.50");
		// $this->printer->lineFeed();

        // $this->printer->setPrintModes(false, false, false);
		// $this->printer->printInColumns("DELIVERY CHARGE", "2.50");
		// $this->printer->lineFeed();

        // $this->printer->setPrintModes(true, true, true);
		// $this->printer->printInColumns("TOTAL", "19.50");
		// $this->printer->lineFeed();
        ...
        // $this->printer->setPrintModes(true, true, true);
        // $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
        // $this->printer->appendText("PAID");
        // $this->printer->lineFeed(2);

        // $this->printer->setPrintModes(false, false, false);
        // $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_LEFT);
        // $this->printer->appendText("Placed : 25/08/23 20:18");
        // $this->printer->lineFeed();
        // $this->printer->appendText("----------------------------------------------");
		// $this->printer->lineFeed();
        // $this->printer->setPrintModes(false, false, false);

        // $this->printer->appendText("STAR KEBAB & Grilled Food Ltd - 408 Portsmouth\n Road, SO19 9AT 8043 6670");
        // $this->printer->lineFeed();

     
        // $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
		// $this->printer->setPrintModes(true, true, true);
		// $this->printer->appendText("34");
		// $this->printer->lineFeed();


        // $this->printer->lineFeed(10);
        $this->printer->cutPaper(false);

        //替换为你设备的SN号
        $sn = $this->serial_number;
        $this->printer->pushContent($sn, sprintf("%s_%010d", $sn, time()));
		$this->printer->clear();
    }
	
	public function ReceiptImageSample($order,$items,$imageUrl)
    {
		$this->printer->selectAsciiCharFont(1);
        $this->printer->setLineSpacing(12);
        $this->printer->setPrintModes(true, true, false);
        // $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
        // $this->printer->appendImage('https://i.pinimg.com/originals/36/cf/a0/36cfa01b357313f964a5d4d07ce63c7b.png', SunmiCloudPrinter::DIFFUSE_DITHER);
        // $this->printer->appendImage($imageUrl, SunmiCloudPrinter::THRESHOLD_DITHER,"500");
        // $this->printer->lineFeed();
		
		 $this->printer->setupColumns(
            [240, SunmiCloudPrinter::ALIGN_LEFT, 0],
            [80, SunmiCloudPrinter::ALIGN_LEFT, 0],
            [80, SunmiCloudPrinter::ALIGN_CENTER, 0],
            [0, SunmiCloudPrinter::ALIGN_RIGHT]
        );
		
		$this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
        $this->printer->setPrintModes(true, false, false);
        $this->printer->printInColumns("ڈالڈا چینی چاول","Price","", "رقم");
        $this->printer->setPrintModes(false, false, false);
        $this->printer->appendText("------------------------------------------");
        $this->printer->lineFeed();
		 $this->printer->printInColumns("ڈالڈا چینی چاول","Price","", "رقم");
    ...
    public function ReceiptSample($order,$items,$imageUrl)
    {
		$this->printer->selectAsciiCharFont(1);
        $this->printer->setLineSpacing(12);
        $this->printer->setPrintModes(true, true, false);
        ...
        $this->printer->printInColumns("Payment", $order['payment_mode']." Payment");
        $this->printer->printInColumns("Receipt No", $order['receipt_no']);
        $this->printer->printInColumns("Customer", $order['bill_print_name']);
        $this->printer->printInColumns("Order", $order['id']);
        $this->printer->printInColumns("Attendant", $order['username']." (".$order['userid'].")");
        $this->printer->printInColumns("TID", $order['terminal_name']);
		...
		$this->printer->appendText("----------------------------------------------");
        $this->printer->lineFeed();
        $this->printer->printInColumns(date("d M Y",strtotime($order['date'])), "",date("h:i a",strtotime($order['time'])));
        ...
        $this->printer->setupColumns(
            [160, SunmiCloudPrinter::ALIGN_CENTER, 0],
            [80, SunmiCloudPrinter::ALIGN_CENTER, 0],
            [80, SunmiCloudPrinter::ALIGN_CENTER, 0],
            [80, SunmiCloudPrinter::ALIGN_CENTER, 0],
            [0, SunmiCloudPrinter::ALIGN_RIGHT,0]
        ...
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
        $this->printer->setPrintModes(true, false, false);
        $this->printer->printInColumns("Product","Unit","Price","Qty", "Amount");
        $this->printer->setPrintModes(false, false, false);
        $this->printer->appendText("----------------------------------------------");
        $this->printer->lineFeed();
		$totalamount = 0;
        ...
        foreach($items as $item){
			$name = str_split($item["item_name"], 25);
			$totalItems++;
			...
			$this->printer->setAlignment(SunmiCloudPrinter::ALIGN_LEFT);
			$this->printer->setPrintModes(true, false, false);
			$this->printer->printInColumns($item["item_name"]);
			$this->printer->setupColumns(
				[160, SunmiCloudPrinter::ALIGN_LEFT, 0],
				[80, SunmiCloudPrinter::ALIGN_RIGHT, 0],
				[80, SunmiCloudPrinter::ALIGN_RIGHT, 0],
				[80, SunmiCloudPrinter::ALIGN_RIGHT, 0],
				[0, SunmiCloudPrinter::ALIGN_RIGHT,0]
			);
			$this->printer->setPrintModes(false, false, false);
			// $this->printer->printInColumns("",$item["uom"], number_format($item["item_price"],0),$item["total_qty"], number_format($item["total_amount"],0));
			$this->printer->printInColumns("",$item["uom"], number_format(5000,0),"250", number_format(50000,0));
			// for($i = 0; $i < count($name); $i++){
				// if($i == 0){
					// $this->printer->printInColumns($name[$i], number_format($item["item_price"],0),$item["total_qty"], number_format($item["total_amount"],0));
				// }else{
					// $this->printer->printInColumns($name[$i], "-", "-", "-");
				// }
			// }
			
			// $this->printer->printInColumns($item["item_name"], number_format($item["item_price"],0),$item["total_qty"], number_format($item["total_amount"],0));
			$this->printer->lineFeed();
		}

        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
		
		$this->printer->setLineSpacing(10);
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
		$this->printer->setupColumns(
            [360, SunmiCloudPrinter::ALIGN_LEFT,0],
            [0, SunmiCloudPrinter::ALIGN_RIGHT,0]
		...
		$this->printer->printInColumns("Item Qty : ".$totalItems, "",$totalQty, number_format($totalamount,0));
		$this->printer->appendText("----------------------------------------------");
		$this->printer->lineFeed();
		$this->printer->setupColumns(
            [360, SunmiCloudPrinter::ALIGN_LEFT,0],
		...
		$this->printer->printInColumns("Gross Amount", number_format($order['total_amount'],0));
		$this->printer->lineFeed();
		$this->printer->printInColumns("Net Amount", number_format($order['total_amount'],0));
		$this->printer->lineFeed();
		$this->printer->printInColumns("Received Amount", number_format($order['receive_amount'],0));
		$this->printer->lineFeed();
		$this->printer->printInColumns("Balance Paid",number_format($order['amount_paid_back'],0));
		$this->printer->lineFeed();
		$this->printer->appendText("----------------------------------------------");
        $this->printer->lineFeed();

        // $this->printer->appendBarcode(SunmiCloudPrinter::HRI_POS_BELOW, 160, 3, 73, $order['receipt_no']);
        // $this->printer->lineFeed();

        // $this->printer->appendQRcode(5, 1, "https://sabsoft.com.pk/Retail/print/$order['receipt_no']");
        // $this->printer->lineFeed();
        ...
        // Print in page mode
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
        $this->printer->appendText("SABIFY KARO SIMPLIFY KARO\n");
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
    ...
    public function KOTSample($order,$department,$departmentItems)
    {
		
        $this->printer->setLineSpacing(10);
        $this->printer->setPrintModes(true, true, false);
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
        $this->printer->appendText("Order KOT Print\n");
        $this->printer->appendText("$department\n");
        $this->printer->appendText("Order # ".$order['id']."\n");
        $this->printer->appendText("-----------------------------------------");
        $this->printer->lineFeed();
        ...
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
        $this->printer->setPrintModes(false, false, false);
        $this->printer->printInColumns("Description", "UOM", "Qty");
		...
		foreach($departmentItems as $item){
			$name = str_split($item["product_name"], 182);
			for($i = 0; $i < count($name); $i++){
				if($i == 0){
					$this->printer->printInColumns($name[$i], $item["uom"], $item["total_qty"]);
				}else{
					$this->printer->printInColumns($name[$i], "-", "-");
				}
			}
        ...
        $this->printer->setLineSpacing(10);
        $this->printer->printInColumns("Cashier ", $order['username']."(".$order['userid'].")");
        $this->printer->lineFeed();
        $this->printer->printInColumns("Receipt # ", $order['receipt_no']);
        $this->printer->lineFeed();
        $this->printer->printInColumns("Terminal ", $order['terminal_name']."(".$order['terminal_id'].")");
        $this->printer->lineFeed();

        $this->printer->appendText("------------------------------------------");
        $this->printer->lineFeed();

        $this->printer->lineFeed(6);
        $this->printer->cutPaper(false);
    ...
    public function ServicePrint($order,$items)
    {
        $this->printer->setLineSpacing(10);
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
       
		$this->printer->setPrintModes(true, true, false);
		$this->printer->appendText("SERVICE PRINT\n");
        $this->printer->appendText("Order # ".$order['id']." (".$order['bill_print_name'].")\n");
        
        $this->printer->setPrintModes(false, false, false);
        $this->printer->setupColumns(
            [144, SunmiCloudPrinter::ALIGN_LEFT, 0],
            [96, SunmiCloudPrinter::ALIGN_CENTER, 0],
            [0, SunmiCloudPrinter::ALIGN_RIGHT, 0]
        );
        $this->printer->appendText("----------------------------------------------");
       ...
       $this->printer->setupColumns(
            [360, SunmiCloudPrinter::ALIGN_LEFT,SunmiCloudPrinter::COLUMN_FLAG_BW_REVERSE],
            [50, SunmiCloudPrinter::ALIGN_RIGHT, SunmiCloudPrinter::COLUMN_FLAG_BW_REVERSE],
            [0, SunmiCloudPrinter::ALIGN_RIGHT,SunmiCloudPrinter::COLUMN_FLAG_BW_REVERSE]
        );
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
        $this->printer->setPrintModes(false, false, false);
        $this->printer->printInColumns("Description", "Qty", "Price");

        $this->printer->setupColumns(
            [360, SunmiCloudPrinter::ALIGN_LEFT,0],
            [50, SunmiCloudPrinter::ALIGN_CENTER,0],
            [0, SunmiCloudPrinter::ALIGN_RIGHT,0]
        );
		...
		$totalItems = 0;
		$totalQty = 0;
		$this->printer->setLineSpacing(15);
		foreach($items as $item){
			$name = str_split($item["product_name"], 28);
			$totalItems++;
			$totalQty = $totalQty + $item["total_qty"];
			$totalamount += $item["total_amount"];
			for($i = 0; $i < count($name); $i++){
				if($i == 0){
					$this->printer->printInColumns($name[$i], $item["total_qty"], $item["total_amount"]);
				}else{
					$this->printer->printInColumns($name[$i], "-", "-");
				}
			}
		...
		// $this->printer->printInColumns("Item Qty", $totalItems);
		$this->printer->printInColumns("Item Qty : ".$totalItems, "",$totalQty, number_format($totalamount,0));
		$this->printer->appendText("----------------------------------------------");
		// $this->printer->setupColumns(
            // [360, SunmiCloudPrinter::ALIGN_LEFT,0],
		...
		// $this->printer->appendText("----------------------------------------------");
		// $this->printer->lineFeed();
		// $this->printer->printInColumns("Total Amount", $totalamount);
		// $this->printer->appendText("----------------------------------------------");
        $this->printer->lineFeed();
        $this->printer->restoreDefaultLineSpacing();
        $this->printer->setAlignment(SunmiCloudPrinter::ALIGN_CENTER);
        $this->printer->setPrintModes(false, false, false);
        $this->printer->setupColumns(
            [250, SunmiCloudPrinter::ALIGN_LEFT,0],
            [0, SunmiCloudPrinter::ALIGN_RIGHT,0]
        );
        $this->printer->setLineSpacing(10);
        $this->printer->printInColumns("Cashier ", $order['terminal_name']."(".$order['terminal_id'].")");
        $this->printer->lineFeed();
        $this->printer->printInColumns("Receipt # ", $order['receipt_no']);
        $this->printer->lineFeed();
        $this->printer->printInColumns("Terminal ", $order['terminal_name']);
		$this->printer->lineFeed();
		$this->printer->appendText("----------------------------------------------");
