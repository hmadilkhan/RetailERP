...
class ReportController extends Controller
{
    ...
    public function invoiceReport(Request $request, Vendor $vendor, Report $report)
    {
        ...
        if ($request->terminalid == 0) {
            ...
            $details = $report->totalSales($request->terminalid, $request->fromdate, $request->todate, $request->type, "all",$request->branch);
        ...
        if ($request->terminalid == 0) {
            ...
            foreach ($terminals as $values) {
                ...
                foreach ($modes as $mode) {
                    ...
                    $details = $report->itemsale_details($request->fromdate, $request->todate, $values->terminal_id, $mode->order_mode_id, $request->department, $request->subdepartment, $request->ordermode, $request->status);
