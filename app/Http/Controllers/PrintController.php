...
class PrintController extends Controller
{
    ...
    public function labelPrintingtwentyfivefivty(Request $request, Vendor $vendor, order $order)
    {
        $company = $vendor->company(session('company_id'));

        $pdf = new labelPrinter(); //'L','mm',array(38,28)
        $pdf->AliasNbPages();
        $pdf->AddPage('L', array(25, 50));
        ...
        if ($request->printheader == "branch") {
            ...
            // $pdf->Cell(0, 6, strtoupper($branch[0]->branch_name), 0, 1, "C");
            $pdf->text(10, 5, strtoupper($branch[0]->branch_name));
        } else {
            $pdf->SetFont('Arial', 'B', 9);
            // $pdf->Cell(45, -3, strtoupper($company[0]->name), 0, 1, "C");
            $pdf->text(20, 5, strtoupper($company[0]->name));
        }

        $pdf->SetXY(5, 5);
        $pdf->text(2, 9, strtoupper($request->name));
        // // $pdf->SetFont('Arial', '', 7);
        // $pdf->Cell(0, -5, strtoupper($request->name), 0, 1, "C");

        $code = 'ABCDEFG123456';
        $pdf->Code128(2, 10, $request->code, 46, 8);

        $labelWidth = 50; // width of the label in mm
        $textWidth = $pdf->GetStringWidth($request->code);
        $x = ($labelWidth - $textWidth) / 2;
        $pdf->text($x, 21, $request->code);
        $pdf->SetXY(5, 5);
        $pdf->text(13, 24, "PRICE : " . number_format($request->price) . " RS");
    ...
    public function labeldoublePrinting1928(Request $request, Vendor $vendor, order $order)
    {
        ...
        if ($request->printheader == "branch") {
            $branch = DB::table("branch")->where("branch_id", session("branch"))->get("branch_name");
            $pdf->cell(8, -15, strtoupper($branch[0]->branch_name), 0, 0, "C");
            $pdf->cell(55, -15, strtoupper($branch[0]->branch_name), 0, 1, "C");
        } else {
            $pdf->cell(8, -15, strtoupper($company[0]->name), 0, 0, "C");
            $pdf->cell(55, -15, strtoupper($company[0]->name), 0, 1, "C");
        }

        $pdf->cell(8, -10, strtoupper($request->name), 0, 1, "C");
        $pdf->text(2, 6, strtoupper($request->name));
        $pdf->text(33, 6, strtoupper($request->name));
        ...
        $pdf->SetXY(5, 5);
        $pdf->text(7, 16, "PRICE : " . number_format($request->price) . " Rs");
        $pdf->text(38, 16, "PRICE : " . number_format($request->price) . " Rs");
    ...
    public function labeltripplePrinting1928(Request $request, Vendor $vendor, order $order)
    {
        ...
        if ($request->printheader == "branch") {
            $branch = DB::table("branch")->where("branch_id", session("branch"))->get("branch_name");
            $pdf->cell(3, -15, strtoupper($branch[0]->branch_name), 0, 0, "C");
            $pdf->cell(33.5, -15, strtoupper($branch[0]->branch_name), 0, 0, "C");
            $pdf->cell(63.5, -15, strtoupper($branch[0]->branch_name), 0, 1, "C");
        } else {
            $pdf->cell(3, -15, strtoupper($company[0]->name), 0, 0, "C");
            $pdf->cell(33.5, -15, strtoupper($company[0]->name), 0, 0, "C");
            $pdf->cell(63.5, -15, strtoupper($company[0]->name), 0, 1, "C");
        }

        $pdf->text($name_margin1, 5, strtoupper($request->name));
        $pdf->text($name_margin2, 5, strtoupper($request->name));
        $pdf->text($name_margin3, 5, strtoupper($request->name)); //2,33.5,63
        ...
        // HERE
        $pdf->SetXY(5, 5);
        $pdf->text(7, 18, "PRICE : " . number_format($request->price) . " ");
        $pdf->text(38.5, 18, "PRICE : " . number_format($request->price) . " ");
        $pdf->text(69.5, 18, "PRICE : " . number_format($request->price) . " ");
    ...
    public function labelPrintingdouble4020(Request $request, Vendor $vendor, order $order)
    {
        ...
        if ($request->printheader == "branch") {
            $branch = DB::table("branch")->where("branch_id", session("branch"))->get("branch_name");
            $pdf->cell(20, -13, strtoupper($branch[0]->branch_name), 0, 0, "C");
            $pdf->cell(71, -13, strtoupper($branch[0]->branch_name), 0, 1, "C");
        } else {
            $pdf->cell(20, -15, strtoupper($company[0]->name), 0, 0, "C");
            $pdf->cell(71, -15, strtoupper($company[0]->name), 0, 1, "C");
        }
        // $pdf->cell(16,-10,strtoupper("Classic Flavor"),0,0,"C");$pdf->cell(16,-10,strtoupper("Classic Flavor"),0,1,"C");
        $pdf->text($name_margin1, 6, strtoupper($request->name));
        $pdf->text($name_margin2, 6, strtoupper($request->name));

        //A set
        $code = 'ABCDEFG123456';
        $pdf->Code128(1, 7, $request->code, 38, 8);
        $pdf->Code128(47, 7, $request->code, 38, 8);
        $textWidth = $pdf->GetStringWidth($request->code);
        $x1 = 1 + (38 - $textWidth) / 2;
        ...
        $pdf->text(55, 22, "PRICE : " . number_format($request->price) . " Rs");
        ...
        if ($general[0]->web) {
            ...
            foreach ($grouped as $deptName => $items) {
                if (!empty($deptName)) {
                    ...
                    <tr></tr>
                    <tr>
                        <td colspan="4" style="text-align: left; font-weight: bold; font-italic: true; background-color: black; color: white; font-size: 10px;text-align: center;" >' . $deptName . '</td>
                    </tr>';
                }
    ...
    <tbody>
        ...
        if ($general[0]->credit_card_transaction > 0) {
            $html .= '
                <tr>
                    <td style="padding:4px; font-weight:bold;font-size:12px;">Card Charges:</td>
                    <td style="padding:4px; text-align:right;font-size:12px;">' . number_format($general[0]->actual_amount / 100 * $general[0]->value, 0) . '</td>
                </tr>';
        }
        ...
        if ($general[0]->delivery_charges > 0) {
            $html .= '
                <tr>
                    <td style="padding:4px; font-weight:bold;font-size:12px;">Delivery Charges:</td>
                    <td style="padding:4px; text-align:right;font-size:12px;">' . number_format($general[0]->charges, 0) . '</td>
                </tr>';
        }
        ...
        if ($general[0]->delivery_charges > 0 || $general[0]->credit_card_transaction > 0 || $general[0]->discount_amount > 0) {
            $html .= '
                <tr>
                    <td style="padding:4px; font-weight:bold;font-size:12px;">Gross Amount:</td>
                    <td style="padding:4px; text-align:right;font-size:12px;">' . number_format($general[0]->actual_amount - $general[0]->discount_amount + ($general[0]->total_amount / 100 * $general[0]->value) + $general[0]->charges, 0) . '</td>
                </tr>';
        }
        ...
        if ($general[0]->sales_tax_amount > 0 || $general[0]->srb > 0) {
            $html .= '
                <tr>
                    <td style="padding:4px; font-weight:bold;font-size:12px;">Sales Tax (' . ($general[0]->sales_tax_amount > 0 ? "FBR" : "SRB") . '):</td>
                    <td style="padding:4px; text-align:right;font-size:12px;">' . number_format(($general[0]->sales_tax_amount > 0 ? $general[0]->sales_tax_amount : $general[0]->srb), 0) . '</td>
                </tr>';
        }

        $html .= '
                <tr>
                    <td style="padding:4px; font-weight:bold;font-size:12px;">Net Amount:</td>
                    <td style="padding:4px; text-align:right;font-size:12px;">' . number_format((float)$general[0]->total_amount, 0) . '</td>
                </tr>';

        if ($general[0]->receive_amount < $general[0]->total_amount) {
            $html .= '
                <tr>
                    <td style="padding:4px; font-weight:bold;font-size:12px;">Received Amount:</td>
                    <td style="padding:4px; text-align:right;font-size:12px;">' . number_format((float)$general[0]->receive_amount, 0) . '</td>
                </tr>
                <tr>
