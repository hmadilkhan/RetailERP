...
class SectionController extends Controller
{
    ...
    public function store(Request $request){
     try{
        ...
        $slug = preg_replace("/[\s_]/", "-", strtolower($request->name));

        $save = Section::create(array_merge(
                $request->except(["_token"]),
                ['slug'=>$slug,'created_at' => date("Y-m-d H:i:s"),'created_at' => date("Y-m-d H:i:s"),'company_id' => session('company_id')]));
     ...
     try{
      ...
      if(Section::customQuery_allColumFetch($conditionApply_two,1,1) > 0){
        ...
        $this->validate($request,$rules);
          ...
          $recordUpdate->slug     = preg_replace("/[\s_]/", "-", strtolower($request->name));
