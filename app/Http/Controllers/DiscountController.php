...
class DiscountController extends Controller
{
    ...
    public function create(discount $discount)
    {
    	...
    	return view("discount.create",compact('discountType','departments','discountvalue'));
    }
    ...
    public function edit(Request $request,discount $discount)
    {
        ...
        $departments = $discount->loadDepartments();
        $discountvalue = $discount->getDiscountValue();
        $discountGen = $discount->getDiscountGeneral(Crypt::decrypt($request->id));


        if ( $discountGen[0]->discount_type == 3) {


            $discountCategories = $discount->getDiscountCategories(Crypt::decrypt($request->id));
            $usage = $discount->getDiscountUsageLimits(Crypt::decrypt($request->id));
            $period = $discount->getDiscountPeroid(Crypt::decrypt($request->id));
            return view("discount.edit-buy",compact('discountType','departments','discountvalue','discountGen','usage','period','discountCategories'));

        }else{

             $discountGenDetails = $discount->getDiscountGeneralDetails(Crypt::decrypt($request->id));
             $usage = $discount->getDiscountUsageLimits(Crypt::decrypt($request->id));
             $period = $discount->getDiscountPeroid(Crypt::decrypt($request->id));
             return view("discount.edit",compact('discountType','departments','discountvalue','discountGen','discountGenDetails','usage','period'));
        }
	...
	{
		$result = $discount->removeDiscount($request->id,$request->mode);
		return $result;
	}
}