...
class ConsolidatedIsdbDatewiseExport  implements WithMultipleSheets
{
	protected $report;
	protected $request;
		...
		return OrderDetails::with("order", "inventory:id,item_code,product_name,weight_qty", "order.terminal:terminal_id,terminal_name", "order.branchrelation:branch_id,branch_name,code")
			->whereHas('order', function ($q) use ($openingIds, $request, $date1, $date2) {
				$q->when(
					...
					function ($q) use ($request, $openingIds) {
	...
	public function sheets(): array
	{
		$sheets = [];
		$maxPage = 10;
		$branch = Branch::with("company:company_id,name")->where("branch_id", $this->request->branch)->first();
		...
		foreach ($intervaldates as $date) {
			...
			$sheets["Date :" . $date] = new ConsolidatedItemSaleReportExport($record, $branch, $datearray);
		}
		return $sheets;
	}
