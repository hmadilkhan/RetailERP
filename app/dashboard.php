...
class dashboard extends Model
{
    ...
    public function sales()
    {
        if (session("roleId") == 2) {
            ...
            $result = DB::select("Select a.terminal_id,a.terminal_name,IFNULL((SELECT SUM(z.total_amount) from sales_receipts z where z.opening_id = b.opening_id and z.payment_id = 1),0) as cash,IFNULL((SELECT SUM(z.total_amount) from sales_receipts z where z.opening_id = b.opening_id and z.payment_id = 2),0) as creditCard,IFNULL((SELECT SUM(z.total_amount) from sales_receipts z where z.opening_id = b.opening_id and z.payment_id = 3),0) as CustomerCredit from terminal_details a INNER JOIN sales_opening b on b.terminal_id = a.terminal_id and b.status=1 where a.branch_id  = ? and a.status_id = 1", [session("branch")]);
            return $result;
        }
    }
    ...
    IFNULL((SELECT SUM(b.actual_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.payment_id = 8 and b.status != 12),0)  as WalletSales,
  ...
  WHERE c.opening_id = a.opening_id
    ...
    public function getheadsDetailsFromOpeningIdForClosing($openingId)
    {
      ...
      WHERE opening_id = a.opening_id
        ...
        IFNULL((SELECT SUM(b.actual_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.payment_id = 1 ),0) as Cash, 
        ...
        IFNULL((SELECT SUM(b.actual_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.payment_id = 8 ),0)  as WalletSales,
        ...
        IFNULL((SELECT SUM(srb+sales_tax_amount) as tax FROM `sales_account_subdetails` where receipt_id IN  (Select id from sales_receipts where opening_id = a.opening_id and void_receipt = 0 and payment_id = 1)),0) as TaxCash,
        IFNULL((SELECT SUM(srb+sales_tax_amount) as tax FROM `sales_account_subdetails` where receipt_id IN  (Select id from sales_receipts where opening_id = a.opening_id and void_receipt = 0 and payment_id = 2)),0) as TaxCard,
        IFNULL((SELECT SUM(srb+sales_tax_amount) as tax FROM `sales_account_subdetails` where receipt_id IN  (Select id from sales_receipts where opening_id = a.opening_id and void_receipt = 0 and payment_id = 3)),0) as TaxCustomerCredit,
