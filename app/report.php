...
class report extends Model
{
    ...
    public function totalSales($terminal, $fromdate, $todate, $type, $category, $customer)
    {
        ...
        if ($type != "" && $type == "datewise") {
            $filter .= " and a.date between '" . $fromdate . "' and '" . $todate . "' and a.terminal_id = " . $terminal;
        } else {
        ...
        if ($customer != "" && $customer != "null" && $customer != "all") {
    ...
    public function sales_details($terminal, $fromdate, $todate)
    {
		...
		IFNULL((SELECT SUM(b.total_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.order_mode_id = 3),0) as Delivery,
		IFNULL((SELECT SUM(b.total_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.order_mode_id = 4),0) as Online,
		IFNULL((SELECT SUM(b.total_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.payment_id = 1 and b.status != 12),0) as Cash, 
		IFNULL((SELECT SUM(b.total_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.payment_id = 2 and b.status != 12),0) as CreditCard, 
		IFNULL((SELECT SUM(b.total_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.payment_id = 3 and b.status != 12),0) as CustomerCredit,
    ...
    public function sales_details_excel_query($branch, $terminal, $fromdate, $todate)
    {
		...
		IFNULL((SELECT SUM(b.total_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.order_mode_id = 3),0) as Delivery,
		IFNULL((SELECT SUM(b.total_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.order_mode_id = 4),0) as Online,
        IFNULL((SELECT SUM(b.total_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.order_mode_id = 2),0) as booking,
		IFNULL((SELECT SUM(b.total_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.payment_id = 1 and b.status != 12),0) as Cash, 
		IFNULL((SELECT SUM(b.total_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.payment_id = 2 and b.status != 12 ),0) as CreditCard, 
		IFNULL((SELECT SUM(b.total_amount) as sales from sales_receipts b where b.opening_id = a.opening_id and b.payment_id = 3 and b.status != 12),0) as CustomerCredit,
		(SELECT SUM(total_cost) FROM sales_receipt_details a INNER JOIN sales_receipts b on b.id = a.receipt_id where b.opening_id = a.opening_id) as cost,
		...
		IFNULL((SELECT SUM(net_amount) FROM `expenses` where opening_id = a.opening_id),0) as Expenses,
		IFNULL((SELECT SUM(amount) FROM sales_return where opening_id = a.opening_id),0) as SalesReturn,
        IFNULL((SELECT SUM(b.actual_amount) AS sales FROM sales_receipts b WHERE opening_id = a.opening_id AND b.void_receipt = 1 and b.payment_id = 1 and b.order_mode_id != 2),0) as voidSalesCash,
        IFNULL((SELECT SUM(b.actual_amount) AS sales FROM sales_receipts b WHERE opening_id = a.opening_id AND b.void_receipt = 1 and b.payment_id != 1 and b.order_mode_id != 2),0) as voidSalesCard,
        IFNULL((SELECT SUM(b.actual_amount) AS sales FROM sales_receipts b WHERE opening_id = a.opening_id AND b.void_receipt = 1 and b.order_mode_id = 2),0) as voidSalesBooking,
		IFNULL((SELECT SUM(amount) as cashout FROM sales_cash_in where terminal_id = a.terminal_id and opening_id = a.opening_id),0) as cashIn,
		IFNULL((SELECT SUM(amount) as cashout FROM sales_cash_out where terminal_id = a.terminal_id and opening_id = a.opening_id),0) as cashOut,
		IFNULL((SELECT SUM(debit) FROM customer_account where opening_id = a.opening_id and payment_mode_id = 1 and received = 1),0) as CashReturn,
		IFNULL((SELECT SUM(debit) FROM customer_account where opening_id = a.opening_id and payment_mode_id = 2 and received = 1),0) as CardReturn,
    ...
    public function itemSalesOrderMode($fromdate, $todate, $terminalid, $mode, $status)
    {
        $filter = "";
    ...
    public function  itemsale_details($fromdate, $todate, $terminalid, $type, $department, $subdepartment = "", $mode = "", $status = "", $inventory = "")
    {
        ...
        // }
        if ($department != "") {
            $filter .= " and c.department_id IN ( " . implode(",", $department) . ")";
        }
        if ($subdepartment != "") {
        ...
        $result = DB::select('SELECT c.id as itemId,c.item_code as code ,c.product_name, SUM(a.total_qty) as qty, SUM(a.total_amount) as amount,AVG(a.item_price) as price, SUM(a.total_cost) as cost,b.void_receipt,c.weight_qty,a.is_sale_return,d.order_status_name,e.order_mode as ordermode,c.department_id,f.department_name FROM sales_receipt_details a INNER JOIN sales_receipts b ON b.id = a.receipt_id INNER JOIN inventory_general c ON c.id = a.item_code INNER JOIN inventory_department f ON f.department_id = c.department_id INNER JOIN sales_order_status d on d.order_status_id = b.status INNER JOIN sales_order_mode e on e.order_mode_id = b.order_mode_id where receipt_id IN (Select id from sales_receipts where opening_id IN (SELECT opening_id FROM `sales_opening` where  date between ? and ? and terminal_id = ? )  and web = 0 ) ' . $filter . ' GROUP BY a.item_code,b.status', [$fromdate, $todate, $terminalid]); //and order_mode_id = ' . $mode . ' and status = ' . $status . '
        return $result;
    }

    public function groupByItemSaleStatus($fromdate, $todate, $terminalid, $type, $department, $subdepartment = "", $mode = "", $status = "")
    {
        $filter = "";
        // if ($department != "") {
        //     $filter .= " and c.department_id IN ( " . implode(",",$department) .")";
        ...
        $result = DB::select('SELECT SUM(a.total_qty) as totalorders,SUM(a.total_amount) as totalamount,d.order_status_name as status FROM sales_receipt_details a INNER JOIN sales_receipts b ON b.id = a.receipt_id INNER JOIN sales_order_mode e on e.order_mode_id = b.order_mode_id INNER JOIN sales_order_status d on d.order_status_id = b.status where receipt_id IN (Select id from sales_receipts where date between ? and ? and terminal_id = ? ' . $filter . ') and b.order_mode_id = ? GROUP BY b.status', [$fromdate, $todate, $terminalid, $type]);
        return $result;
    }
    ...
    public function customerSalesReport($from, $to, $branch, $customer)
    {
        $filter = "";
        if ($branch == "all") {
    ...
    public function orderAmountReceivableTerminalExcel($from, $to, $branch)
    {
        $filter = "";
        if ($branch == "all") {
