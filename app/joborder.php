...
class joborder extends Model
{
    public function getList($statusid)
    {
        $result = DB::table('recipy_general')
            ...
            ->orderBy("recipy_general.recipy_id","desc")
            ->get();
        return $result;
    }
...
//        $uom = DB::select('SELECT a.*, b.*, c.retail_price FROM inventory_general a INNER JOIN inventory_uom b ON b.uom_id = a.uom_id INNER JOIN inventory_price c ON c.product_id = a.id AND c.status_id = a.status WHERE a.id = ?',[$productid]);
    ...
    {
        $result = DB::select("SELECT a.job_order_id, a.joborder_name, a.created_at, b.cost, b.retail_cost, c.job_status_name FROM job_order_general a INNER JOIN job_order_account b ON b.job_id = a.job_order_id INNER JOIN job_status c ON c.job_status_id = a.job_status_id WHERE a.branch_id = ? order by a.job_order_id desc",[session("branch")]);
        return $result;
    }
