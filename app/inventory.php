...
//        $result = DB::statement("UPDATE inventory_price i SET retail_price = $retailprice,actual_price=$actual,tax_rate=$rate,tax_amount=$taxamount,wholesale_price=$wholesale,online_price=$online,discount_price=$discount WHERE i.product_id = $productid AND i.price_id IN  (SELECT price_id FROM (SELECT MAX(price_id) AS price_id FROM inventory_price inP WHERE inP.product_id = $productid AND inP.`status_id` = 1 ) xx ) ");
    ...
    public function get_details($id){
        $inventory = DB::select('SELECT a.*,b.*,c.product_name as mode_name,a.short_description,a.details AS mode,a.inventory_type_id FROM inventory_general a INNER JOIN inventory_qty_reminders b ON b.inventory_id = a.id INNER JOIN inventory_product_mode c ON c.product_mode_id = a.product_mode WHERE a.status = 1 and  a.slug = ?',[$id]);
...
//        ->where('inventory_id',$id)->get();
    ...
    public function get_sub_departments($id)
    {
        if(is_array($id)){
            ...
            // return "SELECT a.* FROM inventory_sub_department a Inner JOIN inventory_department b on b.department_id = a.department_id and b.company_id = ".session("company_id")." where a.department_id IN($values)";;
        }else{
            $values = $id;
        }
        $query = "SELECT a.* FROM inventory_sub_department a Inner JOIN inventory_department b on b.department_id = a.department_id and b.company_id = ".session("company_id")." where a.department_id IN($values)";
        // $result = DB::select('SELECT a.* FROM inventory_sub_department a Inner JOIN inventory_department b on b.department_id = a.department_id and b.company_id = ? where a.department_id IN (?)',[session("company_id"),$values]);
