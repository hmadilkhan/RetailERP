        ...
        foreach ($invoiceSetups as $setup) {
            ...
            if ((int) $company->status_id !== 1) {
                if (!$this->hasOutstandingMonthlyInvoices($company->company_id)) {
                    $skippedCount++;
                    $this->warn("Skipping {$company->name}: company is inactive and has no unpaid monthly invoices.");
                    $auditRows[] = [
                        'company' => $company->name,
                        'invoice' => '-',
                        'generation' => 'skipped',
                        'whatsapp' => 'not_attempted',
                        'detail' => 'Company inactive with no unpaid monthly invoices',
                    ];
                    $this->logBillingRunActivity($runId, 'billing_previous_due_generation', 'generation_skipped', null, [
                        ...
                        'status' => 'skipped',
                        'stage' => 'generation',
                        'reason' => 'Company inactive with no unpaid monthly invoices',
                        'trigger' => 'billing:generate-monthly',
                ...
                try {
                    $invoice = $invoiceGenerationService->generatePreviousDueInvoice($company, $invoiceDate, [
                        'generated_by' => null,
                        'tax_amount' => 0,
                        'notes' => 'Auto-generated previous due invoice for inactive company.',
                        'period_start' => $periodStart,
                        'period_end' => $periodEnd,
                    ]);

                    $generatedCount++;
                    $this->info("Generated previous due invoice {$invoice->invoice_no} for inactive company {$company->name}.");

                    try {
                        $whatsAppResult = $invoiceGenerationService->sendInvoicePdfToWhatsapp($invoice, [
                            'batch_uuid' => $runId,
                            'trigger' => 'billing:generate-monthly:previous-due',
                        ]);

                        if (($whatsAppResult['status'] ?? null) === 'sent') {
                            $whatsAppSentCount++;
                            $this->line("Previous due WhatsApp sent to {$whatsAppResult['to']} for invoice {$invoice->invoice_no}.");
                            $auditRows[] = [
                                'company' => $company->name,
                                'invoice' => $invoice->invoice_no,
                                'generation' => 'previous_due_generated',
                                'whatsapp' => 'sent',
                                'detail' => 'Sent to ' . ($whatsAppResult['to'] ?? 'N/A'),
                            ];
                        } else {
                            $whatsAppSkippedCount++;
                            $this->warn("Previous due WhatsApp skipped for {$company->name}: " . ($whatsAppResult['reason'] ?? 'Unknown reason.'));
                            $auditRows[] = [
                                'company' => $company->name,
                                'invoice' => $invoice->invoice_no,
                                'generation' => 'previous_due_generated',
                        ...
                        $whatsAppFailedCount++;
                        $this->error("Previous due WhatsApp failed for {$company->name}: {$exception->getMessage()}");
                        $auditRows[] = [
                            'company' => $company->name,
                            'invoice' => $invoice->invoice_no,
                            'generation' => 'previous_due_generated',
                    ...
                    $failedCount++;
                    $this->error("Previous due failed for {$company->name}: {$exception->getMessage()}");
                    $auditRows[] = [
                        'company' => $company->name,
                        'invoice' => '-',
                        'generation' => 'failed',
