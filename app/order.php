...
class order extends Model
{
	...
	public function getNewPOSOrdersQuery($request, $mode = "")
	{
		...
		$query = DB::table("sales_receipts")
			...
			->leftJoin("sales_order_mode", "sales_order_mode.order_mode_id", "=", "sales_receipts.order_mode_id")
			->leftJoin("sales_order_status", "sales_order_status.order_status_id", "=", "sales_receipts.status")
			->leftJoin("sales_payment", "sales_payment.payment_id", "=", "sales_receipts.payment_id")
			// ->leftJoin("service_provider_orders", "service_provider_orders.receipt_id", "=", "sales_receipts.id")
			->leftJoin("user_salesprovider_relation as sales_person", "sales_person.user_id", "=", "sales_receipts.sales_person_id")
	...
	protected function getNewPOSOpeningIds($request)
	{
		$fromDate = "";
		$toDate = "";
		if ($request->deli_from != "" && $request->deli_to != "") {
		...
		if (!empty($request->branch) && $request->branch[0] == "all" && !empty($request->terminal) && $request->terminal[0] == "all") {
			return SalesOpening::whereBetween("date", [$fromDate, $toDate])->whereIn("terminal_id", DB::table("terminal_details")->whereIn("branch_id", DB::table("branch")->where("company_id", session("company_id"))->pluck("branch_id"))->pluck("terminal_id"))->pluck("opening_id");
		} else if (!empty($request->branch) &&  $request->branch[0] != "all" && !empty($request->terminal) && $request->terminal[0] == "all") {
			return SalesOpening::whereBetween("date", [$fromDate, $toDate])->whereIn("terminal_id", DB::table("terminal_details")->whereIn("branch_id", DB::table("branch")->whereIn("branch_id", $request->branch)->pluck("branch_id"))->pluck("terminal_id"))->pluck("opening_id");
		}

		return SalesOpening::whereBetween("date", [$fromDate, $toDate])->whereIn("terminal_id", $request->terminal)->pluck("opening_id");
	}

	/** Opening IDs for all company branches (cross-branch mobile lookup; ignores branch/terminal filters). */
	protected function getCompanyWideOpeningIdsForNewPOS($request)
	{
		$fromDate = "";
		$toDate = "";
		if ($request->deli_from != "" && $request->deli_to != "") {
		...
		return SalesOpening::whereBetween("date", [$fromDate, $toDate])
			->whereIn(
				"terminal_id",
				DB::table("terminal_details")
					->whereIn("branch_id", DB::table("branch")->where("company_id", session("company_id"))->pluck("branch_id"))
					->pluck("terminal_id")
			)
			->pluck("opening_id");
	}

	protected function applyNewPOSDateScope($query, $request, $openingIds)
	{
		return $query
			->when($request->first != "" && $request->second != "" && $request->type == "declaration", function ($query) use ($openingIds) {
				$query->whereIn("sales_receipts.opening_id", $openingIds);
			})
			->when($request->first != "" && $request->second != "" && $request->type == "datewise", function ($query) use ($request) {
				$query->whereBetween("sales_receipts.date", [$request->first, $request->second]);
			})
			->when($request->deli_from != "" && $request->deli_to != "" && $request->type == "declaration", function ($query) use ($openingIds) {
				$query->whereIn("sales_receipts.opening_id", $openingIds);
			})
			->when($request->deli_from != "" && $request->deli_to != "" && $request->type == "datewise", function ($query) use ($request) {
				$query->whereBetween(DB::raw('DATE(sales_receipts.order_delivery_date)'), [$request->deli_from, $request->deli_to]);
	...
	/**
	 * @return array<string, array<int, true>> mobile => branch_id => true
	 */
	public function getMobileBranchesInDateRange($request, array $mobiles)
	{
		$mobiles = array_values(array_filter(array_unique($mobiles)));
		if (empty($mobiles)) {
			return [];
		}

		$openingIds = $this->getCompanyWideOpeningIdsForNewPOS($request);
		$query = DB::table("sales_receipts")
			->join("customers", "customers.id", "=", "sales_receipts.customer_id")
			->whereIn("customers.mobile", $mobiles)
			->whereIn('sales_receipts.branch', DB::table("branch")->where("company_id", session("company_id"))->pluck("branch_id"));

		$query = $this->applyNewPOSDateScope($query, $request, $openingIds);

		$rows = $query
			->select("customers.mobile", "sales_receipts.branch")
			->distinct()
			->get();

		$map = [];
		foreach ($rows as $row) {
			if ($row->mobile === null || trim((string) $row->mobile) === '') {
				continue;
			}
			$key = trim((string) $row->mobile);
			$map[$key][(int) $row->branch] = true;
		}

		return $map;
	}

	public function getOrdersByMobileInDateRange($request, $mobile)
	{
	...
	public function orderItemsForPrint($orderID)
	{
		$result = DB::table('sales_receipt_details')
			...
			->where('sales_receipt_details.receipt_id', $orderID)
			->select('inventory_general.id', 'inventory_general.product_name', 'inventory_department.department_name', 'sales_receipt_details.*')
			->get();
		return $result;
	}
	...
	public function getReceiptGeneral($receipt_id)
	{
		$result = DB::select('SELECT a.id as receiptID,a.receipt_no,a.delivery_date,b.order_mode,c.id as customerId,c.name as customerName,c.mobile,c.phone,c.membership_card_no,c.address,a.total_amount,a.actual_amount,d.order_status_name,g.branch_name as branch,g.branch_id as branchId,h.terminal_name,a.date,a.time,c.mobile,e.receive_amount, f.payment_mode,i.*,y.*,z.*,a.machine_terminal_count,t.terminal_name,a.web,a.url_orderid from sales_receipts a
							...
							INNER JOIN terminal_details t on t.terminal_id = a.terminal_id
