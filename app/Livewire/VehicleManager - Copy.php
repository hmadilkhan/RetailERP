...
class VehicleManager extends Component
{
    ...
    public function saveBrand()
    {
        $this->validate([
            ...
            'brandLogo' => $this->brandId ? 'nullable|image|max:1024' : 'required|image|max:1024',
        ]);

        $data = ['name' => $this->brandName];

        if ($this->brandLogo) {
            $data['logo'] = $this->brandLogo->store('brands', 'public');
        }
    ...
    public function saveModel()
    {
        ...
        if ($this->modelLogo) {
            $data['logo'] = $this->modelLogo->store('models', 'public');
        }
