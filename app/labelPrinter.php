...
class labelPrinter extends Fpdf
{
...
//________________ Code 128 encoding and drawing function _____________________
    function Code128($x, $y, $code, $w, $h) {
        ...
        while ($code > "") {
            ...
            $i = strpos($Cguid,$SminiC);                                                // forcing of clearance C, if possible
            if ($i!==false) {
                $Aguid [$i] = "N";
                $Bguid [$i] = "N";
            }

            if (substr($Cguid,0,$IminiC) == $SminiC) {                                  // game C
                $crypt .= chr(($crypt > "") ? $this->JSwap["C"] : $this->JStart["C"]);  // start Cstart, otherwise Cswap
                $made = strpos($Cguid,"N");                                             //extended from set C
                if ($made === false) {
                    $made = strlen($Cguid);
                }
                if (fmod($made,2)==1) {
                    $made--;                                                            // only an even number
                }
                for ($i=0; $i < $made; $i += 2) {
                    $crypt .= chr(strval(substr($code,$i,2)));                          // 2 by 2 conversion
                }
                $jeu = "C";
            } else {
                ...
                $made = (($madeA < $madeB) ? $madeB : $madeA );                         // extended treated
                $jeu = (($madeA < $madeB) ? "B" : "A" );                                // Game in progress
