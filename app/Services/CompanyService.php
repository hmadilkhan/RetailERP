...
class CompanyService
{
    ...
    public function getPaginated(string $search = '', int $perPage = 15)
    {
        $query = DB::table('company as a')
            ->join('city as b', 'b.city_id', '=', 'a.city_id')
            ->join('accessibility_mode as c', 'c.status_id', '=', 'a.status_id')
            ->select('a.company_id', 'a.name', 'b.city_name', 'a.mobile_contact', 'a.email', 'a.address', 'c.status_name', 'a.logo')
            ->where('a.status_id', 1);
        ...
        if ($search !== '') {
            $query->where(function ($q) use ($search) {
                $q->where('a.name', 'like', "%{$search}%")
                    ...
                    ->orWhere('c.status_name', 'like', "%{$search}%");
            });
        }
